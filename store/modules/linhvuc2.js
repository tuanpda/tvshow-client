...
export const getters = {
  getChuongtrinhct: (state) => {
    return state.phattriendoituong;
  },
};

export const mutations = {
  setChuongtrinhct(state, phattriendoituong) {
    state.phattriendoituong = phattriendoituong;
  },
};

export const actions = {
  // Get all data
  async getPhattriendoituong({ commit }) {
    try {
      // Gọi API để lấy danh sách người dùng
      const response = await this.$axios.get("/api/chuongtrinhct/linhvuc2");
  ...
  async createChuongtrinhct({ commit }, data) {
    try {
      ...
      if (response.status == 200) {
        const reload = await this.$axios.get("/api/chuongtrinhct/linhvuc2");
