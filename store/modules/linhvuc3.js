...
export const getters = {
  getChuongtrinhct: (state) => {
    return state.dondocthutaituc;
  },
};

export const mutations = {
  setChuongtrinhct(state, dondocthutaituc) {
    state.dondocthutaituc = dondocthutaituc;
  },
};

export const actions = {
  // Get all data
  async getDondocthutaituc({ commit }) {
    try {
      // Gọi API để lấy danh sách người dùng
      const response = await this.$axios.get("/api/chuongtrinhct/linhvuc3");
  ...
  async createChuongtrinhct({ commit }, data) {
    try {
      ...
      if (response.status == 200) {
        const reload = await this.$axios.get("/api/chuongtrinhct/linhvuc3");
