...
export const getters = {
  getChuongtrinhct: (state) => {
    return state.chuongtrinhct;
  },
};

export const mutations = {
  setChuongtrinhct(state, chuongtrinhct) {
    state.chuongtrinhct = chuongtrinhct;
  },
};

export const actions = {
  // Get all data
  async getChuongtrinhct({ commit }) {
    try {
      // Gọi API để lấy danh sách người dùng
      const response = await this.$axios.get("/api/chuongtrinhct/linhvuc1");
