...
export default {
  ...
  created() {
    this.getPhattriendoituong(); // Gọi action để lấy dữ liệu từ Vuex
  },
  ...
  methods: {
    ...
    async onAdd() {
      ...
      const response = await this.$store.dispatch(
        "modules/linhvuc2/createChuongtrinhct",
        data
      );
    ...
    async onUpdate() {
      ...
      if (result.isConfirmed) {
        try {
          ...
          // cập nhật dữ liệu
          const response = await this.$store.dispatch(
            "modules/linhvuc2/updateChuongtrinhct",
            payload
          );
    ...
    async onDelete(data) {
      ...
      if (result.isConfirmed) {
        try {
          const response = await this.$store.dispatch(
            "modules/linhvuc2/deleteChuongtrinhct",
            data
          );
          if (response.success == true) {
            const Toast = Swal.mixin({
